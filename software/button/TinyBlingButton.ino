...
// Description:
// ------------
// Various decorative light animations using the TinyBling's neopixels. You can switch
// between the different animations by pressing the button.
//
// References:
// -----------
...
int main(void) {
  ...
  while(1) {
    // Animate
    switch(state) {
      case 0:   hue1 = 0; hue2 = 96; ptr1 = 0; ptr2 = 8; state++;
                break;
    ...
    if(~PINB & (1<<BT_PIN)) {             // if button pressed:
      if(++state > 7) state = 0;          // switch to next animation
